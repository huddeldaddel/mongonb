...
class IndexManager {
    ...
    /**
     * @return {@code null} if there are no duplicate indexes - an error message if there are duplicates.
     */
    ...
    public String hasEmptyIndexes() {
        final List<Integer> emptyIndexes = new LinkedList<Integer>();
        int pos = 0;
        for(Index index: model.getIndexes()) {
            pos++;
    ...
    public void modifyCollection() {
        for(Index index: model.getIndexesToDelete())
            model.getCollection().dropIndex(index);
        for(Index index: model.getIndexesToCreate())
            try {
                model.getCollection().createIndex(index);
